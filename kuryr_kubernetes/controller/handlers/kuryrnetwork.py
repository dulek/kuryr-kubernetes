...
class KuryrNetworkHandler(k8s_base.ResourceEventHandler):
    ...
    def on_present(self, kuryrnet_crd):
        # Check if the CRD is mark for deletion
        if kuryrnet_crd['metadata'].get('deletionTimestamp'):
            self.delete_namespace_resources(kuryrnet_crd)
            return
        ...
        if not net_id:
            ...
            status = {'netId': net_id}
            self._patch_kuryrnet_crd(kuryrnet_crd, status)
            crd_creation = True
        ...
        if (crd_creation or
                ns_labels != kuryrnet_crd['spec'].get('nsLabels', {})):
            ...
            # update status
            status = {'nsLabels': kuryrnet_crd['spec'].get('nsLabels', {})}
            self._patch_kuryrnet_crd(kuryrnet_crd, status, labels=True)
    ...
    def _patch_kuryrnet_crd(self, kuryrnet_crd, status, labels=False):
        kubernetes = clients.get_kubernetes_client()
        LOG.debug('Patching KuryrNet CRD %s', kuryrnet_crd)
        try:
            if labels:
            ...
            LOG.debug('KuryrNet CRD not found %s', kuryrnet_crd)
        except k_exc.K8sClientException:
            LOG.exception('Error updating kuryrnetwork CRD %s', kuryrnet_crd)
            raise

    def delete_namespace_resources(self, kuryrnet_crd):
