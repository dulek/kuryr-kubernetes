...
class TestNamespaceHandler(test_base.TestCase):
    ...
    def test_on_present(self):
        ...
        self._get_kns_crd.assert_called_once_with(
            net_crd['metadata']['name'], self._namespace['metadata']['name'])
        self._set_net_crd.assert_called_once_with(self._namespace,
                                                  net_crd['metadata']['name'])
        self._add_kuryrnet_crd.assert_called_once_with(
            self._namespace['metadata']['name'], net_crd['metadata']['name'],
            {})
    ...
    def test_on_present_existing(self):
        ...
        self._add_kuryrnet_crd.assert_not_called()

    def test_on_present_annotated_crd(self):
        net_crd = self._get_crd()
        self._get_kns_crd.return_value = None
        ns = self._namespace.copy()
        ns['metadata']['annotations'] = {
            constants.K8S_ANNOTATION_NETWORK_CRD: net_crd['metadata']['name']}

        namespace.NamespaceHandler.on_present(self._handler, ns)
    ...
    def test_on_present_set_net_crd_exception(self):
        net_crd = self._get_crd()
        self._get_kns_crd.return_value = None
    ...
    def test_on_deleted(self):
        ...
        self._del_kuryrnet_crd.assert_called_once_with(
            net_crd['metadata']['name'], net_crd['metadata']['namespace'])

    def test_on_deleted_missing_crd(self):
        self._get_kns_crd.return_value = None
        namespace.NamespaceHandler.on_deleted(self._handler, self._namespace)
